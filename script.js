...
// Create dense grass with no text elements
function createDenseGrassNoText() {
    console.log("Creating dense grass with no text...");

    try {
        // Remove any existing text elements
        const textElements = document.querySelectorAll('[data-version-indicator], [data-progress-bar], [data-ui-container], [data-instructions]');
        ...
        grassContainer.name = "Dense Grass No Text";
        grassContainer.userData.isGrass = true;
        
        console.log("Creating dense instanced grass...");
        
        const planeGeometry = new THREE.PlaneGeometry(0.5, 0.5);
        
        const terrainSize = 30;
        const halfSize = terrainSize / 2;
        const gridSize = 0.5; // Smaller grid for increased density
        const maxInstances = 10000; // Increased instance count for density
