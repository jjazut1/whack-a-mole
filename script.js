...
// Create optimized texture-based grass terrain
function createTextureBasedGrassTerrain() {
    console.log("Creating optimized texture-based grass terrain...");
    
    try {
        // Add version indicator
        const versionNumber = "12.0.0";
        const versionTimestamp = new Date().toISOString();
        ...
        indicator.style.fontFamily = 'monospace';
        indicator.style.zIndex = '1000';
        indicator.textContent = `Texture-Based Grass v${versionNumber}`;
        document.body.appendChild(indicator);
        ...
        window.gameVersionInfo = {
            version: versionNumber,
            timestamp: versionTimestamp,
            feature: "Optimized Texture-Based Grass"
        };
        ...
        // Look for the main terrain plane
        let terrainPlane = null;
        scene.traverse(object => {
            // Look for the likely terrain plane - large flat object near y=0
            if (object.type === 'Mesh' && 
                object.geometry && 
                (object.geometry.type === 'PlaneGeometry' || 
                 object.geometry.type === 'PlaneBufferGeometry') &&
                Math.abs(object.position.y) < 1) {
        ...
        const terrainSize = 30; // Default terrain size
        ...
        if (!terrainPlane) {
            ...
            // Create material - we'll update this with our grass texture
            const material = new THREE.MeshLambertMaterial({
                color: 0x7CFC00, // Base green color
                side: THREE.DoubleSide
            });
        ...
        // Get hole positions to create the mask
        const holePositions = [];
        scene.children.forEach(child => {
            if (child.geometry && 
                child.geometry.type === 'CircleGeometry') {
        ...
        // Create a canvas for the composite grass texture
        const textureSize = 2048; // High resolution for detailed grass
        const canvas = document.createElement('canvas');
        ...
        // Create the grass texture with various visual techniques
        
        // Function to convert world coordinates to canvas coordinates
        function worldToCanvas(x, z) {
            // Convert from world space (-halfSize to halfSize) to canvas space (0 to textureSize)
            const halfSize = terrainSize / 2;
            const canvasX = ((x + halfSize) / terrainSize) * textureSize;
            const canvasZ = ((z + halfSize) / terrainSize) * textureSize;
            return { x: canvasX, y: canvasZ }; // Note: z in world = y in canvas
        }
        
        // Draw grass with noise patterns
        function drawGrassTexture() {
            // Create a pattern of thin blades
            const bladeCount = 120000; // Lots of blades for a dense appearance
            
            // Create various blade colors
            const grassColors = [
                '#388E3C', // Dark green
            ...
            // First pass - background texture and variation
            ctx.globalCompositeOperation = 'source-over';
            
            // Add a bit of noise and texture to the base
            for (let i = 0; i < textureSize; i += 4) {
                for (let j = 0; j < textureSize; j += 4) {
                    // Random subtle color variation
                    const variation = Math.random() * 20 - 10;
                    const r = 76 + variation; // Base on medium green (RGB 76,175,80)
                    const g = 175 + variation/2;
                    const b = 80 + variation;
            ...
            // Second pass - individual grass blades
            for (let i = 0; i < bladeCount; i++) {
                // Random position
                const x = Math.random() * textureSize;
                const y = Math.random() * textureSize;
                
                // Convert to world coordinates to check for holes
                const worldX = ((x / textureSize) * terrainSize) - (terrainSize / 2);
                const worldZ = ((y / textureSize) * terrainSize) - (terrainSize / 2);
                
                // Skip if in a hole
                let inHole = false;
                for (let hole of holePositions) {
                    const dx = worldX - hole.x;
                    const dz = worldZ - hole.z;
                    const distanceSquared = dx * dx + dz * dz;
                ...
                const length = 3 + Math.random() * 12; // Blade length
                const width = 1 + Math.random() * 1.5;  // Blade width
                ...
                // Use bezier curve for more natural blade shape
                ctx.beginPath();
                ctx.moveTo(-width/2, 0);
                
                // Control point for curve
                const curveOffset = (Math.random() - 0.5) * 3;
                
                ctx.bezierCurveTo(
                    -width/4, length/3,
                    curveOffset, length/2,
                    0, length
                );
            ...
            // Third pass - Add some highlights and shadows for depth
            ...
            for (let i = 0; i < 5000; i++) {
                ...
                // Randomly use light or dark to create depth
                if (Math.random() > 0.7) {
                    ctx.fillStyle = 'rgba(255, 255, 255, 0.1)'; // Subtle highlight
                } else {
                    ctx.fillStyle = 'rgba(0, 0, 0, 0.1)'; // Subtle shadow
                }
            ...
            // Lastly, draw the holes as masked areas
            ctx.fillStyle = 'rgba(0, 0, 0, 0)'; // Transparent
            
            // Draw each hole
            for (let hole of holePositions) {
                const canvasPos = worldToCanvas(hole.x, hole.z);
                const canvasRadius = (hole.radius / terrainSize) * textureSize;
        ...
        // Create a normal map for added depth
        const normalMapCanvas = document.createElement('canvas');
        normalMapCanvas.width = textureSize;
        normalMapCanvas.height = textureSize;
        const normalCtx = normalMapCanvas.getContext('2d');
        
        // Copy the grass texture to start
        normalCtx.drawImage(canvas, 0, 0);
        
        // Convert to grayscale & create normal map effect
        const imageData = normalCtx.getImageData(0, 0, textureSize, textureSize);
        const data = imageData.data;
        
        // Simple normal map calculation
        for (let y = 1; y < textureSize - 1; y++) {
            for (let x = 1; x < textureSize - 1; x++) {
                const index = (y * textureSize + x) * 4;
                const up = (y - 1) * textureSize * 4 + x * 4;
                const down = (y + 1) * textureSize * 4 + x * 4;
                const left = y * textureSize * 4 + (x - 1) * 4;
                const right = y * textureSize * 4 + (x + 1) * 4;
                
                // Simplified normal calculation based on neighboring pixels
                const dx = data[right] - data[left];
                const dy = data[down] - data[up];
                
                // Convert to normal RGB format (128,128,255 is flat)
                data[index] = 128 + dx / 2;     // R = X normal
                data[index + 1] = 128 + dy / 2; // G = Y normal
                data[index + 2] = 200;          // B = Z normal (mostly up)
        ...
        // Apply textures to terrain material
        if (terrainPlane && terrainPlane.material) {
            ...
            // Ensure the terrain receives shadows
            terrainPlane.receiveShadow = true;
            
            console.log("Applied grass texture to terrain");
        }
        
        // Add some subtle 3D details to enhance the texture-based approach
        function addGrassDetails() {
            // Add a few 3D grass clumps in the foreground for depth
            const detailCount = 200;
            const detailGroup = new THREE.Group();
            detailGroup.name = "Grass 3D Details";
            
            // Simple grass blade geometry
            const bladeGeometry = new THREE.PlaneGeometry(0.1, 0.3);
            
            // Loop through and create detail grass in the foreground
            for (let i = 0; i < detailCount; i++) {
                // Position in a half-circle in front of the camera
                ...
                const y = 0.1; // Small offset from ground
                ...
                // Create crossed blades
                const blade1 = new THREE.Mesh(bladeGeometry, material);
                blade1.position.set(x, y, z);
                blade1.rotation.y = Math.random() * Math.PI;
                
                const blade2 = new THREE.Mesh(bladeGeometry, material);
                blade2.position.set(x, y, z);
                blade2.rotation.y = blade1.rotation.y + Math.PI / 2;
                
                // Random scale
                const scale = 0.7 + Math.random() * 0.6;
                blade1.scale.set(scale, scale, scale);
                blade2.scale.set(scale, scale, scale);
                
                detailGroup.add(blade1);
                detailGroup.add(blade2);
            }
        ...
        // Add 3D grass details for depth
        const detailGroup = addGrassDetails();
        
        // Final success message
        console.log("Texture-based grass terrain created successfully");
        
        // Return created objects for reference
        return {
        ...
        console.error("Error creating texture-based grass:", e);
        return null;
    }
}

// Call the function
createTextureBasedGrassTerrain();
