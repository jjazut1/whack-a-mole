...
// Create optimized green grass with top-positioned indicators
function createOptimizedGreenGrassWithTopUI() {
    console.log("Creating performance-optimized green grass with top UI...");
    
    try {
        // Add version indicator at the TOP of the page
        const versionNumber = "11.6.0";
        const versionTimestamp = new Date().toISOString();
        
        // Remove any existing indicators
        const existingIndicators = document.querySelectorAll('[data-version-indicator], [data-progress-bar]');
        existingIndicators.forEach(el => el.remove());
        
        // Create UI container at the top
        const uiContainer = document.createElement('div');
        uiContainer.setAttribute('data-ui-container', 'true');
        ...
        const indicator = document.createElement('div');
        indicator.setAttribute('data-version-indicator', 'true');
        indicator.style.background = 'rgba(255, 152, 0, 0.8)'; // Orange during generation
        indicator.style.color = 'white';
        ...
        indicator.innerHTML = `
            <span>Optimizing Grass Performance...</span>
            <span style="font-size: 12px; opacity: 0.8;">v${versionNumber}</span>
        `;
        uiContainer.appendChild(indicator);
        
        // Instructions text
        const instructions = document.createElement('div');
        instructions.setAttribute('data-instructions', 'true');
        instructions.style.background = 'rgba(33, 33, 33, 0.8)';
        ...
        instructions.textContent = "Performance-optimized grass using instanced rendering. Maintains visual quality while reducing GPU load.";
        uiContainer.appendChild(instructions);
        
        // Progress bar container
        const progressContainer = document.createElement('div');
        progressContainer.setAttribute('data-progress-bar', 'true');
        ...
        window.gameVersionInfo = {
            version: versionNumber,
            timestamp: versionTimestamp,
            feature: "Performance-Optimized Grass with Top UI"
        };
        
        console.log(`Version ${versionNumber} - ${versionTimestamp}`);
        
        // Remove existing grass first
        let grassRemoved = 0;
        ...
        // Create optimized grass textures
        function createGrassTextures() {
            // Create a canvas to draw grass blades
            const canvas = document.createElement('canvas');
            canvas.width = 256;
            canvas.height = 256;
            ...
            // Draw grass blades (triangular shape)
            for (let i = 0; i < 50; i++) {
            ...
            // Create a canvas for the second crossed texture
            ...
            // Draw grass blades at different angle
            for (let i = 0; i < 50; i++) {
                const x = Math.random() * canvas2.width;
                const y = Math.random() * (canvas2.height / 2); // Start higher up
                const width = 5 + Math.random() * 12;
                const height = 60 + Math.random() * 150;
                
                // Select random green
                ctx2.fillStyle = greenColors[Math.floor(Math.random() * greenColors.length)];
                
                // Draw a blade at an angle
                ctx2.beginPath();
                ctx2.moveTo(x, canvas2.height - y);
                ctx2.lineTo(x - width/2, canvas2.height - y);
                ctx2.lineTo(x + 20, canvas2.height - y - height);
                ctx2.lineTo(x + width/2, canvas2.height - y);
                ctx2.closePath();
                ctx2.fill();
            }
        ...
        updateProgress(5, "Creating grass textures...");
        
        // Create optimized materials using the textures
        ...
        updateProgress(15, "Analyzing terrain...");
        ...
        updateProgress(25, "Creating grass container...");
        
        // Create a master container for all grass
        const grassContainer = new THREE.Group();
        grassContainer.name = "Optimized Grass Container";
        grassContainer.userData.isGrass = true;
        
        // OPTIMIZATION 1: Use instanced meshes for identical grass planes
        updateProgress(35, "Generating instanced grass...");
        ...
        const terrainSize = 30;
        const halfSize = terrainSize / 2;
        const gridSize = 1.0; // Larger grid for reduced instance count
        const maxInstances = 5000; // Cap instances for performance
        ...
        updateProgress(50, "Positioning grass instances...");
        ...
        updateProgress(75, "Adding detail patches...");
        
        // OPTIMIZATION 2: Add detail with far fewer individual patches
        console.log("Adding detail patches...");
        
        // Create a small number of detail patches in key areas (edges, etc.)
        const detailPatchCount = 50; // Much smaller than before
        const detailPatches = new THREE.Group();
        detailPatches.name = "Detail Grass Patches";
        
        for (let i = 0; i < detailPatchCount; i++) {
            // Position primarily around edges for visible detail
            let posX, posZ;
            const edgeProximity = Math.random() < 0.7; // 70% near edges
            
            if (edgeProximity) {
                // Position near edges
                if (Math.random() < 0.5) {
                    posX = (Math.random() < 0.5 ? -1 : 1) * (halfSize * 0.8 + Math.random() * halfSize * 0.2);
                    posZ = (Math.random() * 2 - 1) * halfSize;
                } else {
                    posX = (Math.random() * 2 - 1) * halfSize;
                    posZ = (Math.random() < 0.5 ? -1 : 1) * (halfSize * 0.8 + Math.random() * halfSize * 0.2);
                }
            } else {
                // Random position in terrain
                posX = (Math.random() * 2 - 1) * halfSize;
                posZ = (Math.random() * 2 - 1) * halfSize;
            }
            ...
            // Create a detail patch (crossed planes with different scale/rotation)
            const patchSize = 0.6 + Math.random() * 0.4;
            
            // Vertical plane
            const planeV = new THREE.Mesh(planeGeometry, grassMaterial);
            planeV.position.set(posX, posY + patchSize * 0.25, posZ);
            planeV.rotation.y = Math.random() * Math.PI;
            planeV.scale.set(patchSize, patchSize, patchSize);
            
            // Horizontal plane (crossed)
            const planeH = new THREE.Mesh(planeGeometry, grassMaterial2);
            planeH.position.copy(planeV.position);
            planeH.rotation.y = planeV.rotation.y + Math.PI / 2;
            planeH.scale.copy(planeV.scale);
            
            detailPatches.add(planeV);
            detailPatches.add(planeH);
        }
        
        grassContainer.add(detailPatches);
        
        updateProgress(90, "Optimizing rendering performance...");
        
        // OPTIMIZATION 3: Add the container to the scene
        scene.add(grassContainer);
        
        // OPTIMIZATION 4: Make grass static for maximum performance
        grassContainer.matrixAutoUpdate = false;
        grassContainer.updateMatrix();
        
        // OPTIMIZATION 5: Apply additional performance optimizations
        ...
        updateProgress(100, "Grass optimization complete!");
        ...
        setTimeout(() => {
            ...
            indicator.innerHTML = `
                <span>Optimized Grass Complete</span>
                <span style="font-size: 12px; opacity: 0.8;">v${versionNumber}</span>
            `;
            
            // Update instructions
            instructions.textContent = "Performance-optimized grass is active. Using instanced rendering with 5000+ grass elements for maximum efficiency.";
        ...
        console.log("Optimized grass generation complete");
        return grassContainer;
        
    } catch (e) {
        console.error("Error creating optimized grass:", e);
        
        // Error notification in UI
        const uiContainer = document.querySelector('[data-ui-container]');
        if (uiContainer) {
            const errorMessage = document.createElement('div');
            errorMessage.style.background = 'rgba(244, 67, 54, 0.8)';
...
// Call the function
createOptimizedGreenGrassWithTopUI();
